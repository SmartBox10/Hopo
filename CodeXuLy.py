...
def tra_tu(khachhang):
   for tu in Tu.TuList:
       if tu.ten_khach_hang == khachhang.ten_dang_nhap:
           ...
           khachhang.tien_phai_tra = round(khachhang.thoi_gian_su_dung * 5000)
           if (khachhang.thoi_gian_su_dung)>2 or (khachhang.thoi_gian_su_dung == 2):
            ...
            return 5#trả lố thời gian quy định
   return 0  # Khách hàng chưa đặt tủ

def tra_tu1(ma_nhap,khachhang):#Khi trả tủ lố thời gian quy định
   if khachhang.kiem_tra_ma_dinh_danh(ma_nhap) == 1:
       ...
       return 2  # Trả chậm, đã đóng tiền và nhập mã định danh đúng. quay về trang login
   else:
